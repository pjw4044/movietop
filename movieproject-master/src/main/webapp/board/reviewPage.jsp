...
<%@ page import="board.BoardCommentBean" %>
<%@ page import="board.BoardLikeBean" %>
...
<%
	...
	if(flag!=null){
		if(flag.equals("insert")){
			BoardCommentBean cbean = new BoardCommentBean();
		...
		{
			mgr.pushLike(num, loginedid);
			flag=null;
		}
		else if(flag.equals("clike"))
		{
			mgr.cancelLike(num, loginedid);
			flag=null;
		}
	}else{
	...
	Vector<BoardCommentBean> cvlist=mgr.getCommentList(num);
	String title=bean.getTitle();
	//String posted=null;
	String posted=bean.getPostedDate().toString();
	String bCat=bean.getCategory();
...
<table align="center" width="600" cellspacing="3">
 <tr>
  <td bgcolor="#DDDDDD" height="25" align="center">글읽기</td>
 </tr>
 <tr>
  <td colspan="2">
 ...
 <tr> 
    ...
    <td bgcolor="#FFFFFF" colspan="3"><%="<"+bean.getCategory()+">"+title%></td>
   </tr>
   <%
   ...
   <td>
   <button id="g-button">
   <%if(mgr.hasLiked(num, loginedid)){ %>
	<img id="g-image" src="img/Fgood.png" alt="Button Image" width="30px" height="30px">
   <%}else{ %>
...
{
	BoardCommentBean cbean=cvlist.get(i);
	int cnum=cbean.getcIdx();
	String cname=cbean.getUserId();
	//String cname="1q2w3e4r";
	String comment=cbean.getDetail();
	String cmem=memgr.getUserNm(cname);
...
%>
 ...
 <!-- 댓글 List End -->
 [ <a href="javascript:list()" >리스트</a> <%if(((isLogined && loginedid.equals(postedId)) || adminEd) && !(bean.getCategory().equals("공지"))) {%>| 
 <a href="updateReviewPage.jsp?nowPage=<%=nowPage%>&num=<%=num%>&numPerPage=<%=numPerPage%>" >수 정</a> |
...
<script type="text/javascript">
	function list() {
		document.listFrm.action = "boardPage.jsp";
