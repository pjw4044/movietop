...
public class db {
    ...
    public String load(int page, int year) {
    	Connection con = null;
    	...
    	try {
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                // 이미지 파일 저장
                con = pool.getConnection();
    			sql = "INSERT movie(TITLE, CONTENT, POSTER, GENRE, DIRECTOR, ACTOR, PLAYTIME, AGE, TRAILER, WATCHOTT, MAKER, opendt)"
                + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
