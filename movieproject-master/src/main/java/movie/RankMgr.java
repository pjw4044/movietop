...
public class RankMgr {
	...
	//idx 사용해서 영화명이랑 포스터 가져오기 
	public MovieBean getRecommend(int idx){
		String posterUrl = "https://image.tmdb.org/t/p/w500";
		MovieBean bean = new MovieBean();
		Connection con = null;
		PreparedStatement pstmt = null;
	...
	//영화 좋아요 랭킹
	public List<MovieBean> siteMovieRank() {
		List<MovieBean> vlist = new ArrayList<MovieBean>();
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql = "select movieidx, count(movieidx) as cnt "
					+ "from likedmovie "
					+ " group by movieidx order by cnt desc LIMIT 5";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) {
			
				idx = rs.getInt("movieidx");
				MovieBean bean = getRecommend(idx);
				vlist.add(bean);
